...
- **`final_delivery` se comporta como un módulo normal (jul 2026):** antes `nodeStatus` exigía **TODOS** los módulos completos para habilitar la entrega (`others.every(done)`), lo que la bloqueaba si estaba **en medio** de la ruta con módulos después. Ahora usa sus `req` explícitos o, por defecto, el **módulo anterior** en el orden — igual que cualquier módulo. En paralelo, `Grid.jsx` calcula `routeComplete` solo sobre los módulos **antes** de la entrega (`modsBeforeFinal`), no sobre todos. Para una entrega al final de la ruta el comportamiento es equivalente al de siempre.
