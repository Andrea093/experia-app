...
- **La entrega se marca COMPLETADA al enviarla, y NO genera certificado (jul 2026):** `Grid.handleSubmit` llama `completeNode(finalMod.id)` tras `submitProduct`/`resubmitProduct`, para que el nodo de entrega quede completo (desbloquea el siguiente y suma al 100%). El diploma **ya no se emite en el Grid**: el bloque de `status==='approved'` solo muestra una confirmación (antes renderizaba `CertificatePage` → `issueCertificate`). El certificado lo emite **solo** el nodo `certificate` → `CourseCertificatePage` al completar el 100% (`maybeIssueCourseCertificate`). `CertificatePage` en Grid.jsx quedó como código muerto (ya no se renderiza).
