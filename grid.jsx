// =============================================
// EXPERIA — Student Product Upload + Instructor Review
// =============================================
    ...
    <div style={{ height: '100%', overflow: 'auto', WebkitOverflowScrolling: 'touch', padding: isMobile ? '16px 12px 48px' : '32px 24px 60px', background: 'var(--bg)' }}>
      ...
      <div id="cert-wrap" style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div id="certificate" style={{
          background: 'white', border: isMobile ? '5px solid var(--orange)' : '8px solid var(--orange)',
          borderRadius: isMobile ? 16 : 24,
          padding: isMobile ? '32px 20px' : '56px 64px', textAlign: 'center', position: 'relative',
          boxShadow: 'var(--sh-xl)',
        }}>
...
const StudentProductUpload = () => {
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--warn)', fontWeight: 600, marginTop: 2 }}>
                  ...
                  {existingSub.returnCount < 2
                    ? ' — Aún puedes recibir una devolución más si es necesario'
                    : ' — Esta es tu última oportunidad de corrección'}
                </div>
              </div>
            </div>
            ...
            {(existingSub.instrRejillaName || existingSub.instrPreguntaName) && (
              <div style={{ padding: '12px 16px', borderRadius: 10, background: '#EFF6FF', border: '1px solid #BFDBFE', marginTop: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#1D4ED8', textTransform: 'uppercase', letterSpacing: .5, marginBottom: 8 }}>
                  📎 Archivos corregidos por el instructor:
                </div>
                  ...
                  {existingSub.instrRejillaName && (
                    <button onClick={() => downloadFile(existingSub.instrRejillaName, existingSub.instrRejillaData)}
                      style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8,
                        border: '1px solid #BFDBFE', background: 'white', cursor: 'pointer', fontFamily: 'var(--font)',
                        fontSize: 13, color: '#1D4ED8', fontWeight: 500, textAlign: 'left' }}>
                      📄 {existingSub.instrRejillaName}
                      <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 4 }}>↓ Descargar</span>
                    </button>
                  )}
  ...
  // === ALREADY SUBMITTED (pending / graded / or just submitted correction) ===
...
const InstructorDashboard = () => {
  ...
  // Group by area (for filtered view)
  ...
  // Group by institution → area (for "all institutions" view)
    ...
    <div style={{ height: '100%', overflow: 'auto', WebkitOverflowScrolling: 'touch', padding: isMobile ? '0 16px 40px' : '0 24px 40px' }}>
      ...
      {institutions.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: .8, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            🏫 Institución
          </div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => { setActiveInstitution('all'); setActiveArea('all'); }}
              style={{ padding: '7px 14px', borderRadius: 8, border: 'none', cursor: 'pointer',
                fontFamily: 'var(--font)', fontSize: 12, fontWeight: 600, transition: 'all .2s',
                background: activeInstitution === 'all' ? 'var(--dark)' : 'var(--bg-alt)',
                color: activeInstitution === 'all' ? '#fff' : 'var(--muted)' }}>
            ...
            {institutions.map(inst => {
              ...
              return (
                <button key={inst} onClick={() => { setActiveInstitution(inst); setActiveArea('all'); }}
                  style={{ padding: '7px 14px', borderRadius: 8, border: isActive ? '1.5px solid var(--purple)' : '1px solid var(--border)',
                    cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 12, fontWeight: 600, transition: 'all .2s',
                    background: isActive ? 'var(--purple-bg)' : 'var(--white)',
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        <button onClick={() => setActiveArea('all')} style={{ padding: '7px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 12, fontWeight: 600, background: activeArea === 'all' ? 'var(--dark)' : 'var(--bg-alt)', color: activeArea === 'all' ? '#fff' : 'var(--muted)', transition: 'all .2s' }}>
          Todas ({totalCount})
        </button>
        {AREAS.map(a => {
          const count = grouped[a.id]?.length || 0;
          return <button key={a.id} onClick={() => setActiveArea(a.id)} style={{ padding: '7px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 12, fontWeight: 600, background: activeArea === a.id ? a.color + '18' : 'var(--bg-alt)', color: activeArea === a.id ? a.color : 'var(--muted)', transition: 'all .2s' }}>
            {a.icon} {a.name} ({count})
          </button>;
        })}
      </div>

      {/* Submissions — grouped by institution when showing all, by area otherwise */}
              ...
              return (
                <div key={inst} style={{ marginBottom: 32 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14,
                    padding: '10px 16px', borderRadius: 12, background: 'var(--purple-bg)',
                    border: '1px solid var(--purple-light)' }}>
                    <span style={{ fontSize: 20 }}>🏫</span>
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--purple-deep)', flex: 1 }}>{inst}</h3>
                    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--purple)', background: 'var(--white)',
                      padding: '2px 10px', borderRadius: 6, border: '1px solid var(--purple-light)' }}>
                      {instTotal} entrega{instTotal !== 1 ? 's' : ''}
                    </span>
                  </div>
                  {/* Areas within institution */}
                  {AREAS.map(area => {
                    const subs = areaMap[area.id] || [];
                    if (subs.length === 0) return null;
                    return (
                      <div key={area.id} style={{ marginBottom: 16, marginLeft: 0 }}>
        ...
        {currentSub && (
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, padding: '12px 16px', borderRadius: 10, background: 'var(--bg-alt)' }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--orange-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: 'var(--orange)' }}>
                {currentSub.studentName.charAt(0)}
              </div>
              <div style={{ flex: 1 }}>
                ...
                {currentSub.studentInstitution && (
                  <div style={{ fontSize: 11, color: 'var(--purple)', fontWeight: 600, marginTop: 2 }}>
                    🏫 {currentSub.studentInstitution}
                  </div>
                )}
              </div>
              {(currentSub.returnCount || 0) > 0 && (
                <div style={{ padding: '4px 12px', borderRadius: 8, background: '#FEF9E7', border: '1px solid #FCD34D', fontSize: 12, fontWeight: 700, color: '#92400E', whiteSpace: 'nowrap' }}>
                  ↩️ Devuelto {currentSub.returnCount}/2
                </div>
              )}
            </div>
            ...
            {(() => {
              ...
              const renderFiles = (entry) => (
                <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 6 }}>
                  {[
                    { name: entry.rejillaName, data: entry.rejillaData, color: 'var(--orange)' },
                    { name: entry.preguntaName, data: entry.preguntaData, color: 'var(--purple)' },
                  ].map((f, i) => (
                    <button key={i} onClick={() => openFile(f.name, f.data)}
                      style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 8,
                        border: '1px solid var(--border)', background: 'var(--white)', cursor: 'pointer',
                        fontFamily: 'var(--font)', fontSize: 12, fontWeight: 600, color: 'var(--dark)', transition: 'all .2s' }}
                      onMouseEnter={e => e.currentTarget.style.borderColor = f.color}
                      onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
                      <FileIc s={13} c={f.color} /> {f.name || '—'}
                      {f.data
                        ? <span style={{ fontSize: 9, color: 'var(--success)', fontWeight: 700 }}>↗ Ver</span>
                        : <span style={{ fontSize: 9, color: 'var(--subtle)' }}>sin datos</span>}
                    </button>
                  ))}
                </div>
              );
              ...
              if (history.length === 0) {
                return (
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: .8, marginBottom: 8 }}>
                      Archivos entregados
                    </div>
                    {renderFiles(currentSub)}
                  </div>
                );
              }
              ...
              return (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--dark)' }}>📋 Historial de entregas</span>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 5, background: 'var(--purple-bg)', color: 'var(--purple)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {history.map((entry, i) => (
                      <div key={i} style={{ padding: '10px 14px', borderRadius: 10, background: 'var(--bg)', border: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 4, background: '#FEF3C7', color: '#92400E', fontWeight: 700 }}>
                            Devuelta
                          </span>
                        </div>
                        {renderFiles(entry)}
                    ...
                    <div style={{ padding: '10px 14px', borderRadius: 10, background: 'var(--orange-bg)', border: '1.5px solid var(--orange-light)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--orange)' }}>
                          Versión {history.length + 1} · {currentSub.date}
                        </span>
                        <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 4, background: 'var(--orange)', color: '#fff', fontWeight: 700 }}>
                          Actual
                        </span>
                      </div>
                      {renderFiles(currentSub)}
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[1, 2, 3, 4, 5].map(v => (
                      <button key={v} onClick={() => setGradeValues(g => ({ ...g, [cr.key]: v }))}
                        style={{ width: 36, height: 36, borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 14, transition: 'all .15s', background: (gradeValues[cr.key] || 0) >= v ? 'var(--orange)' : 'var(--bg-alt)', color: (gradeValues[cr.key] || 0) >= v ? '#fff' : 'var(--muted)' }}>{v}</button>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={feedbackText} onChange={e => setFeedbackText(e.target.value)}
                ...
                rows={3} style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1.5px solid var(--border)', fontFamily: 'var(--font)', fontSize: 14, resize: 'vertical', outline: 'none', transition: 'border-color .2s', boxSizing: 'border-box' }}
                onFocus={e => e.target.style.borderColor = 'var(--orange)'}
                onBlur={e => e.target.style.borderColor = 'var(--border)'} />
            </div>

            {/* Return section (only if allowed) */}
            {canReturn && (
              <div style={{ padding: '16px', borderRadius: 12, background: '#FFFBEB', border: '1.5px solid #FCD34D', marginBottom: 20 }}>
                ...
                <p style={{ fontSize: 12, color: '#92400E', marginBottom: 8, lineHeight: 1.5 }}>
                  Escribe qué debe corregir el estudiante. Al devolver, la calificación se borrará y el estudiante podrá subir archivos nuevamente.
                </p>
                <textarea value={returnNotes} onChange={e => setReturnNotes(e.target.value)}
                  placeholder="Ej: Revisar la formulación de la pregunta, mejorar la alineación con los estándares del área..."
                  rows={3}
                ...
                <div style={{ marginTop: 12 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#92400E', marginBottom: 8 }}>
                    📎 Adjuntar archivos corregidos al estudiante (opcional)
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                {canReturn && (
                  <button onClick={handleReturn} disabled={!returnNotes.trim()}
                    style={{ padding: '8px 16px', borderRadius: 10, border: '1.5px solid #FCD34D', background: returnNotes.trim() ? '#FFFBEB' : 'var(--bg-alt)', color: returnNotes.trim() ? '#92400E' : 'var(--subtle)', fontFamily: 'var(--font)', fontSize: 13, fontWeight: 600, cursor: returnNotes.trim() ? 'pointer' : 'not-allowed', transition: 'all .2s' }}>
                    ↩️ Devolver para corrección
                  </button>
                )}
                <Btn variant="gradient" size="sm" onClick={handleApprove}>
                  ✅ Aprobar proyecto
                </Btn>
              </div>
            </div>
      ...
      <Modal open={!!viewFile && !viewFile?.data} onClose={() => setViewFile(null)} title="Archivo" width={400}>
        <div style={{ textAlign: 'center', padding: '20px' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--muted)' }}>
            Este archivo fue registrado pero los datos no están disponibles en esta sesión.
            Solo los archivos subidos en la sesión actual pueden visualizarse.
          </p>
        </div>
      </Modal>
    </div>
...
Object.assign(window, { StudentProductUpload, InstructorDashboard, SubTable, CertificatePage });
